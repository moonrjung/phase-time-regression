...
Vendored verbatim from AlignBeat's own alignbeat/downsample.py -- one of the
pieces the move to continuous phase regression does not change, since it sits
entirely below the heads. Copied rather than imported so this folder stays
self-contained (importing the alignbeat package pulls in the earlier draft's
3-class SubsetSelectionHead transitively).

This replaces the hand-rolled EncoderToCandidates in heads.py. Two practical
consequences beyond using the code that is already trained and tested:

  * its Conv1d stack is built in __init__, from a fragment_frames known at
    construction time -- so the weights exist before an optimizer captures
    model.parameters(). EncoderToCandidates built them lazily inside forward()
    and they were therefore never trained.
  * strides multiply to T/N exactly (factor_strides) rather than rounding up
    to powers of two, so t_hat = 1 keeps meaning the end of the window.
"""
import math
